...
if [ ! -e $SERVER_JARLIST_PATH\server$mc_donwload_version.jar ]; then
    ...
    while :; do
        ...
        [yY]*)
            ...
            wget -q https://launcher.mojang.com/v1/objects/0252918a5f9d47e3c6eb1dfec02134d1374a89b4/server.jar -O $SERVER_JARLIST_PATH\server$mc_donwload_version.jar
            SPINNER_FILECHECK
            if [ -e $SERVER_JARLIST_PATH\server$mc_donwload_version.jar ]; then
                JARDOWNLOADSUCCESS
                break
